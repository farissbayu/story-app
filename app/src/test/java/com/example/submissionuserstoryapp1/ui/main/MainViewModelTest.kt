...
import com.example.submissionuserstoryapp1.DataDummy
import com.example.submissionuserstoryapp1.MainDispatcherRule
...
import com.example.submissionuserstoryapp1.getOrAwaitValue
...
class MainViewModelTest{
    ...
    private val token = "Bearer authxtoken"

    @Test
    fun `when success load story data`() = runTest {
        ...
        Mockito.`when`(storyRepository.getListStory(token)).thenReturn(expectedStory)

        val mainViewModel = MainViewModel(storyRepository, userRepository)
        val actualStory: PagingData<StoryItem> = mainViewModel.getListStory(token).getOrAwaitValue()
    ...
    @Test
    fun `when no story data`() = runTest {
